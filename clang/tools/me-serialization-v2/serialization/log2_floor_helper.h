...
#if defined(__GNUC__)
  template<typename T>
  static std::enable_if_t<Bits<T, sizeof(std::size_t)>::value == 8, unsigned int> Log2Floor(T value) {
    return 63 ^ static_cast<uint64_t>(__builtin_clzll(value));
  }
  template<typename T>
  static std::enable_if_t<Bits<T, sizeof(std::size_t)>::value == 4, unsigned int> Log2Floor(T value) {
    return 31 ^ static_cast<uint32_t>(__builtin_clz(value));
  }
#elif defined(_MSC_VER)
  template<typename T>
  static
  std::enable_if_t<Bits<T, sizeof(std::size_t)>::value == 8, unsigned int> Log2Floor(T value) {
       unsigned long where;
    ...
    template<typename T>
  static
  std::enable_if_t<Bits<T, sizeof(std::size_t)>::value == 4, unsigned int> Log2Floor(T value) {
       unsigned long where;
